...
{
    public partial class PermissionForm : XtraForm
    {
        ...
        {
            string filePath = $"{Application.StartupPath}\\Template\\Permission.xlsx";
        ...
        private async void btn_Save_Click(object sender, EventArgs e)
        {
            ...
            await SQLCRUD.ExecuteAsync(@"IF OBJECT_ID('dbo.AKTARIZIN', 'U') IS NOT NULL
BEGIN
    DROP TABLE dbo.AKTARIZIN;
            END;
            ...
            await SQLCRUD.ExecuteAsync("TRUNCATE TABLE  AKTARIZIN");
            for (int i = 0; i < gridView1.RowCount; i++)
            {
              string personelCode = gridView1.GetRowCellValue(i, gridView1.Columns[0])?.ToString();
                string izinCode = gridView1.GetRowCellValue(i, gridView1.Columns[1])?.ToString();
                string izinTanimi = gridView1.GetRowCellValue(i, gridView1.Columns[2])?.ToString();
                string izinAciklamasi = gridView1.GetRowCellValue(i, gridView1.Columns[3])?.ToString();
                string izinBaslangic = gridView1.GetRowCellValue(i, gridView1.Columns[4])?.ToString();
                string izinBitis = gridView1.GetRowCellValue(i, gridView1.Columns[5])?.ToString();
                string sure = gridView1.GetRowCellValue(i, gridView1.Columns[6])?.ToString();
                string sureTipi = gridView1.GetRowCellValue(i, gridView1.Columns[7])?.ToString();
                await SQLCRUD.ExecuteAsync($@" INSERT INTO AKTARIZIN (PersonelCode, IzinCode, IzinTanimi, IzinAciklamasi, IzinBaslangic, IzinBitis, Sure, SureTipi) VALUES ('{personelCode}', '{izinCode}', '{izinTanimi}', '{izinAciklamasi}', '{izinBaslangic}', '{izinBitis}', {sure}, '{sureTipi}')");
            }
            bool status=await SQLCRUD.ExecuteAsync("EXEC ASY_VacTransfer");
            if (status)
            {
        ...
        {
            await Backup.TableBackUp("vactransfer.txt");
        }
    }
}
...
{
    public partial class PlaceForm : XtraForm
    {
        public PlaceForm()
        {
            InitializeComponent();
        ...
        {
            string filePath = $"{Application.StartupPath}\\Template\\Place.xlsx";
        ...
        {
            await Backup.TableBackUp("place.txt");
        }
        ...
        private async void btn_Save_Click(object sender, EventArgs e)
        {
            ...
            await SQLCRUD.ExecuteAsync(@"IF OBJECT_ID('dbo.AKTARPLACE', 'U') IS NOT NULL
BEGIN
    DROP TABLE dbo.AKTARPLACE;
            END;
            ...
            await SQLCRUD.ExecuteAsync("TRUNCATE TABLE  AKTARPLACE");
            for (int i = 0; i < gridView1.RowCount; i++)
            {
                string personelCode = gridView1.GetRowCellValue(i, gridView1.Columns[0])?.ToString();
                string placeCode = gridView1.GetRowCellValue(i, gridView1.Columns[1])?.ToString();
                string placeDesc = gridView1.GetRowCellValue(i, gridView1.Columns[2])?.ToString();
                await SQLCRUD.ExecuteAsync($@" INSERT INTO AKTARPLACE (PERSONCODE, PLACECODE, PLACEDESC) VALUES ('{personelCode}', '{placeCode}', '{placeDesc}')");
            }
            bool status = await SQLCRUD.ExecuteAsync("EXEC ASY_PERSON_PLACE");
            if (status)
            {

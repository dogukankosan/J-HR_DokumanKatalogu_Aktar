...
{
    public partial class JobForm :XtraForm
    {
        public JobForm()
        {
            InitializeComponent();
        ...
        {
            string filePath = $"{Application.StartupPath}\\Template\\Job.xlsx";
        ...
        {
            await Backup.TableBackUp("job.txt");
        }
        private async void btn_Save_Click(object sender, EventArgs e)
        {
            ...
            await SQLCRUD.ExecuteAsync(@"IF OBJECT_ID('dbo.AKTARJOB', 'U') IS NOT NULL
BEGIN
    DROP TABLE dbo.AKTARJOB;
            END;
            ...
            await SQLCRUD.ExecuteAsync("TRUNCATE TABLE  AKTARJOB");
            for (int i = 0; i < gridView1.RowCount; i++)
            {
                string personelCode = gridView1.GetRowCellValue(i, gridView1.Columns[0])?.ToString();
                string jobCode = gridView1.GetRowCellValue(i, gridView1.Columns[1])?.ToString();
                string jobDesc = gridView1.GetRowCellValue(i, gridView1.Columns[2])?.ToString();
                await SQLCRUD.ExecuteAsync($@" INSERT INTO AKTARJOB (PERSONCODE, JOBCODE, JOBDESC) VALUES ('{personelCode}', '{jobCode}', '{jobDesc}')");
            }
            bool status = await SQLCRUD.ExecuteAsync("EXEC ASY_PERSON_JOB");
            if (status)
            {
